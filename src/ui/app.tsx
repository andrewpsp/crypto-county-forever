...
import { CryptoCountyWrapper } from '../lib/contracts/CryptoCountyWrapper';
...
export function App() {
    ...
    const [contract, setContract] = useState<CryptoCountyWrapper>();
    ...
    async function deployContract() {
        const _contract = new CryptoCountyWrapper(web3);
    ...
    async function createNFT() {
        try {
            ...
            toast('Successfully created Crypto-County Asset', { type: 'success' });
        } catch (error) {
            console.error(error);
            toast.error('Failed create nft');
        } finally {
            setTransactionInProgress(false);
        }
    }
    ...
    return (
        <div>
            ...
            <button onClick={createNFT} disabled={!contract}>
                create Crypto-County Assets
            </button>
            <br />
            <div>
                <h4> List Crypto-County Assets</h4>
                {listNFT.map(_uri => (
                    <img
                        key={_uri}
                        src={_uri}
